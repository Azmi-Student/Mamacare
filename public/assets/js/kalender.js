    ...
    fetch(
        ...
        .then((nutrisi) => {
            ...
            Swal.fire({
                ...
                html: `
    <div id="pdf-content" class="swal-responsive"
         style="display:flex; gap:20px; flex-wrap:wrap; text-align:left; font-size:15px; max-height:500px; overflow-y:auto;">
      <div>
        <h3>📄 Info Kehamilan</h3>
        <p><strong>📅 Tanggal HPHT:</strong><br>${data.hphtDate.toLocaleDateString("id-ID")}</p>
        <p><strong>⏳ Usia Kehamilan:</strong><br>
          <span style="color:#d63384;font-weight:bold;">
            ${data.minggu} Minggu ${data.hari} Hari
          </span>
        </p>
        <p><strong>👶 Perkiraan Lahir:</strong><br>${data.formattedDue}</p>
        <p><strong>🌱 Trimester:</strong><br>${data.trimester}</p>
      ...
      <div>
        ...
        <ul>${rekomendasiList}</ul>
        <p><strong>⚠️ Dihindari:</strong></p>
        <ul>${hindariList}</ul>
      </div>
    </div>

    <!-- Tombol download ditaruh DI DALAM popup -->
  ...
  `,
                ...
                didRender: () => {
                    document.getElementById("btnDownloadPDF").addEventListener("click", () => {
                        ...
                        downloadSwalAsPDF(container, `informasi_kehamilan_${data.minggu}w_${data.hari}d.pdf`);
                    });
                }
            });
...
function tampilkanSimulasiKehamilan(data) {
    Swal.fire({
        ...
        html: `
            ...
            <p style="font-size: 18px; font-weight: bold;">👶 ${data.formattedDue
            }</p>
            <br>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const updateSlider = (selector, value, max) => {
                ...
                group.querySelector(".progress-icon").style.left = `calc(${(value / max) * 100
                    }% - 12px)`;
...
function downloadSwalAsPDF(domNode, filename = 'informasi_kehamilan.pdf') {
    if (!domNode) return;

    // Clone biar bisa tweak kecil tanpa ngubah UI Swal
    const clone = domNode.cloneNode(true);

    // Optional: hilangkan scrollbar di clone agar rapi
    clone.style.maxHeight = 'unset';
    clone.style.overflow = 'visible';
    clone.style.width = '100%';

    // Bungkus ke container sementara agar html2pdf dapat ukuran stabil
    const wrapper = document.createElement('div');
    wrapper.style.padding = '20px';
    wrapper.style.background = '#fff';      // background putih untuk PDF
    wrapper.style.color = '#000';           // kontras aman
    wrapper.appendChild(clone);
    document.body.appendChild(wrapper);

    // Opsi kualitas tinggi + page break aman
    const opt = {
        margin: [10, 12, 10, 12],       // top, right, bottom, left (pt)
        filename,
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: {
            scale: 2,                            // naikin DPI biar tajam
            useCORS: true,                       // biar gambar eksternal bisa dirender
            backgroundColor: '#ffffff',
            scrollX: 0,
            scrollY: -window.scrollY
        },
        jsPDF: { unit: 'pt', format: 'a4', orientation: 'portrait' },
        pagebreak: { mode: ['css', 'legacy'] } // dukung class .html2pdf__page-break
    };

    // Render dan bersihkan elemen sementara
